...
def pick_adress_sport_pages() :
    ...
    """
    reponse = requests.get("https://www.3x3ffbb.com/news/")
    html=reponse.content
    soup=bs(html, "lxml")
    section = soup.find("div", {"class" : "news-content"})
    items=section.find_all("div", {"class" : "news-item"})
    print("items", items)
    urls=[]
    names=[]
    for item in items:
        print("item :",item,"\n")
        name= item.find("div", {"class":"post-name"}).text
        print( 'name : ')
        names.append(name)
        url = item.a["href"]
        print("url : ",url,)
...
def content_article(url, file_output):
    """scrapp content  web page in a file and the plain code
       ...
       url : adress web page of Basketball(3x3)  French federation
